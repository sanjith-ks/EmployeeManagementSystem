...
public class SalaryInfoDaoImpl implements SalaryInfoDao {
	...
	@Override
	public List<SalaryInfo> sortBySalaryDao() {
		List<SalaryInfo> salarylist = db.query("select * from employee.employee_salary_information order by Monthly_Salary", new ResultSetExtractor<List<SalaryInfo>>() {
	...
	@Override
	public String updateSalaryInfoDao(SalaryInfo s) {
		String result="Data updated Successfully.";
		String sql="update employee.employee_salary_information set Monthly_Salary=?,"
				+ "salary_cycle=?,paid_salary=? where Employee_ID_Number=?";
		int records=0;
		try {
			records=db.update(sql,s.getMonthlySalary(),s.getSalary_cycle(),s.getPaid_salary(),s.getEmployeeIDNumber());
		}
		catch(Exception e) {
			System.out.println(e.getStackTrace());
